...
/**
 * Validate that a value is a plausible UUID video identifier.
 * Rejects empty strings and obvious junk (e.g. "undefined", "null")
 * to avoid sending bogus IDs to backend /videos/:videoId/ endpoints.
 *
 * Exported so UI modules (IndexingRecords, FootagePlayback, dialogs)
 * can share exactly the same definition of "valid video id".
 */
 ...
 *
 * Backend contract (VideoCommitIn):
 *   {
 *     videoId: string,
 *     s3KeyRaw: string,
 *     fileName: string,
 *     frameStride?: number,          // default 3
 ...
 *   })
 */
export async function vaCommit(
  wid,
  {
    videoId,
    s3KeyRaw,
  ...
  if (!isValidVideoId(videoId)) {
    throw new Error("vaCommit: valid videoId is required")
  }
  if (!s3KeyRaw) {
    throw new Error("vaCommit: s3KeyRaw is required")
  }
  if (!fileName) {
    throw new Error("vaCommit: fileName is required")
  }
  if (!cameraCode) {
    throw new Error("vaCommit: cameraCode is required")
  }
...
export async function vaEnqueue(wid, videoId, body = { variant: "cmt" }) {
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaEnqueue] Invalid videoId: ${videoId}`)
  }
...
export async function vaShow(wid, videoId) {
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaShow] Invalid videoId: ${videoId}`)
  }
...
export async function vaUpdate(wid, videoId, patch) {
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaUpdate] Invalid videoId: ${videoId}`)
  }
...
export async function vaDelete(wid, videoId, body) {
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaDelete] Invalid videoId: ${videoId}`)
  }
...
export async function vaRuns(wid, videoId) {
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaRuns] Invalid videoId: ${videoId}`)
  }
...
export async function vaProgress(
  ...
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaProgress] Invalid videoId: ${videoId}`)
  }

  const params = new URLSearchParams()
  if (variant) params.set("variant", String(variant))
  // Backend progress endpoint currently ignores runId, but we keep the name aligned
...
export async function vaDetections(
  ...
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaDetections] Invalid videoId: ${videoId}`)
  }
...
export async function vaDetection(
  ...
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaDetection] Invalid videoId: ${videoId}`)
  }
...
export async function vaDetectionUpdate(
  ...
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaDetectionUpdate] Invalid videoId: ${videoId}`)
  }
...
export async function vaDetectionsAll(
  ...
  for (const v of list) {
    ...
    if (!isValidVideoId(v.id)) {
      console.warn(
        "[videoAnalysis] vaDetectionsAll skipping invalid video id:",
        v.id
      )
      continue
    }
...
export async function vaVideoUrl(
  ...
  if (!isValidVideoId(videoId)) {
    throw new Error(`[vaVideoUrl] Invalid videoId: ${videoId}`)
  }
